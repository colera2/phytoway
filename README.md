...
├── 데이터 DB 업로드 자동화.py              # 로컬 엑셀 파일 → DB 배치 업로드 (초기방식)
...
##### 로컬 엑셀 → DB 배치 업로드 (초기방식)
수동으로 다운로드한 엑셀 파일을 DB에 적재하는 프로세스 입니다.
```
...
```
><sub>초기 운영 방식으로, 이후 파일 업로드 웹 솔루션으로 고도화되었습니다. (별도 repository [링크](https://github.com/colera2/my-repo))</sub>
...
# 로컬 엑셀 파일 DB 업로드 (초기방식 - 현재는 웹 솔루션으로 대체)
