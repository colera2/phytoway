"""
네이버 검색광고 API - 쇼핑 검색어 상세 보고서 수집 스크립트

[동작 순서]
1. 네이버 검색광고 API로 쇼핑 검색어 상세 보고서 생성 요청
...
def get_shoppingkeyword_detail_data():
    """
    네이버 검색광고 API를 통해 쇼핑 검색어 상세 보고서를 생성하고 다운로드.
    ...
    try:
        ...
        # 예: 오늘이 2024-11-26이면 stat_date = '20241125'
        ...
        print(f"get_shoppingkeyword_detail_data 함수에서 오류 발생: {e}")
        return None
...
def insert_data_into_sql(db_info, table_name, data, insert_query):
    ...
    try:
        ...
        for record in data:
            try:
                print(record)
                # 현재 시간 추가 (reg_tm, edit_tm) - 필요 시 주석 해제하여 사용
...
def insert_shopping_keyword_detail_table(data):
    """
    보고서 데이터를 파싱하여 naverapi_shopping_keyword_details 테이블에 삽입.

    Args:
        data (list): 보고서 줄 단위 리스트 (탭 구분자, 16개 컬럼)
    """
    if not data:
        print("삽입할 데이터가 없습니다.")
        return
    ...
    for idx, line in enumerate(data):
        ...
        # 컬럼 수가 16개가 아니면 해당 줄 건너뜀
        if len(columns) != 16:
            print(f"데이터 형식 오류 (줄 번호 {idx + 1}): 예상 16개, 실제 {len(columns)}개. 데이터: {line}")
            continue
        ...
        try:
            ...
            transformed_data = [
                ...
                int(columns[7]),                                 # hours (시간대)
                columns[8],                                      # regionCode (지역 코드)
                columns[9],                                      # mediaCode (매체 코드)
                columns[10].strip(),                             # pcMobileType (PC/모바일 구분)
                int(columns[11]),                                # impression (노출수)
                int(columns[12]),                                # click (클릭수)
                float(columns[13]),                              # cost (비용)
                int(columns[14]),                                # sumAdRank (광고 순위 합)
                int(columns[15]),                                # viewCount (조회수)
            ]
            parsed_data.append(transformed_data)
            # print(transformed_data)  # 디버깅 시 주석 해제
        except Exception as e:
